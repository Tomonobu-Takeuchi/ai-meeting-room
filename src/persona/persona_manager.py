...
・ユーザーが語った「取り組みたい理由」「これまで続かなかった理由」を踏まえ、自信を失っている様子なら小さな達成から積み上げる伝え方を、他者からの評価を重視している様子なら成果を共有する機会を意識した伝え方を選ぶ
...
"""
    ...
    def build_facilitator_prompt(self, facilitator, topic, history_text, mode='guide', member_ids=None, crisis_mode=False, category=None, phase='diverge'):
        # 収束プロトコル：カテゴリ別 必須確認スロット（定義表v1 §3）
        opening_slots = {
            'strategy':     "「事業・アイデアの内容」「想定しているお客様のイメージ」「使える時間・資金・人脈などの制約」「1年後にどうなっていたいか」",
            'practice':     "「提案の概要と結論」「決裁者は誰で何を最も重視する人か」「提案の期限」「過去に指摘された弱点」",
            'consulting':   "「現在の仕事と経験年数」「家計・家族など動かせない条件」「何に不安や不満を感じているか」",
            'relationship': "「相手との関係と経緯」「その関係を今後も維持したいか」「本当に得たいもの」「話がまとまらなかった場合の代替案の有無」",
            'study':        "「何を学び・作りたいか」「現在のレベル」「週に確保できる時間」「完成した姿のイメージ」「なぜそれに取り組みたいと思ったか（理由）」",
        }

        # --- 収束プロトコル共通ブロック（guide/nominateで共用） ---
        convergence_block = ""
        if phase == 'converge':
            # 収束プロトコル：カテゴリ別の絞り込み対象と評価軸（定義表v1 §3）
            gate_map = {
                'strategy':     ("ターゲット市場", "市場規模・自社の強みとの適合・競合の強さ・冒頭に確認した制約との整合"),
                'practice':     ("当日の合否を分ける最重要の反論（3つ）", "決裁者への影響度・発生確率・回答準備の難しさ"),
                'consulting':   ("進むべき進路シナリオ", "10年後の後悔の小ささ・実現可能性・動かせない条件との整合"),
                'relationship': ("交渉のゴールと譲歩ライン", "関係維持の重要度・実利の大きさ・代替案の強さ"),
                'study':        ("90日で完成させる最初の成果物", "確保できる時間内での完成可能性・学習効果・完成の喜びまでの距離"),
            }
            gate = gate_map.get(category)
            gate_text = (
                f"③絞り込みの関門：実行の詳細に入る前に、必ず「{gate[0]}」の候補を3つ以内に整理し、"
                f"評価軸（{gate[1]}）で比較を添えて提示し、【質問】でユーザーにどれにするか選択を求めてください。"
                "ユーザーが選ぶまで、実行の詳細（具体的な施策・計画）には進まないでください。"
                "ユーザーが決められない場合は、評価軸のうちどれを最も重視するかを先に選んでもらってください。\n"
            ) if gate else ""
            convergence_block = (
                "\n【収束フェーズの進行ルール】\n"
                "議論が中盤に入りました。以下を優先してください。\n"
                "①決定台帳：整理の際は、これまでに決まったことを番号付きで列挙し、各項目に"
                "【確定】（ユーザーが明言・選択したもの）または【仮】（ペルソナの提案にユーザーがまだ明確に同意していないもの）を付けてください。"
                "ユーザーが明確に同意するまで【仮】を【確定】と書いてはいけません。会議で誰も口にしていない数値や案を台帳に書くことは禁止です。\n"
                + gate_text +
                "④変更の扱い：ユーザーの発言が【確定】済みの事項と食い違う場合、黙って上書きせず、"
                "変更の意図かを一度確認し、変更する場合は影響を受ける他の決定事項を1つ2つ挙げてから台帳を更新してください。\n"
                "⑤逸れた論点は、発言の価値を認めた上でリフレームして本筋に戻してください。"
                "介入は否定ではなくリフレームで行い、メンバーの発言の面白さを損なわないこと。\n"
            )

        if mode == 'opening':
            slots = opening_slots.get(category)
            slot_text = (
                f"この会議で良い結論を出すために、{slots} を、会話の流れの中で自然に確認してください。"
                "一度に全部聞かず、まず1〜2点から尋ねてください。150字以内で。\n"
            ) if slots else "「今の状況」と「特に気になっていること」の2点を、\n1〜2文のシンプルな言葉で聞いてください。100字以内で。\n"
            instruction = (
            ...
            if phase == 'converge':
                instruction += convergence_block + (
                    "\n【指名より優先する関門】絞り込みの選択をまだユーザーに求めていない場合、"
                    "または未選択のまま実行の詳細に入ろうとしている場合は、ペルソナを指名せず、"
                    "決定台帳と候補比較を提示して【質問】でユーザーに選択を求めてください"
                    "（この場合【指名:】タグは書かないこと）。"
                    "既にユーザーが選択済みの場合は、台帳を添えたうえで通常どおり指名してください。"
                    "収束フェーズでは100字制限より台帳の提示を優先し、500字以内とします。\n"
                )
        elif mode == 'summarize':
            ...
            category_guide = {
                ...
                'study': (
                    ...
                    "あわせて「なぜそれに取り組みたいと思ったのですか」「これまで途中でやめてしまった時、何が一番つらかったですか」も自然な会話の中で聞き、"
                    "動機の背景（誰かに認められたい・自分の力を試したい・純粋に楽しいから等）と過去の挫折体験を把握する。\n"
...
通常は、表・見出し・絵文字・太字・箇条書き記号などのMarkdown装飾を使わず、地の文で300字以内に簡潔にまとめてください。ただし、決定台帳の提示または絞り込み候補の比較提示を行うときに限り、「1. 2. 3.」形式の番号付きリストを使用してよく、500字以内まで認めます（表・見出し・絵文字・太字は引き続き使わないこと）。
