...
function openPersonaTosModal() {
  const overlay = document.getElementById('personaTosOverlay');
  if (overlay) {
    const check = document.getElementById('personaTosAgreeCheck');
    const btn = document.getElementById('personaTosAgreeBtn');
...
function closePersonaTosModal() {
  const overlay = document.getElementById('personaTosOverlay');
  if (overlay) overlay.classList.add('hidden');
}

async function agreePersonaTos() {
  closePersonaTosModal();
  localStorage.setItem('persona_tos_agreed', '1');
  // T-06: 故人チェックが入っている場合は故人同意モーダルへ
...
async function init() {
  ...
  try {
    ...
    // T-08: personaTosAgreeCheckのchangeイベントでボタン活性化
    document.getElementById('personaTosAgreeCheck')?.addEventListener('change', e => {
      const btn = document.getElementById('personaTosAgreeBtn');
      if (btn) btn.disabled = !e.target.checked;
    });
    // 著名人チェックでインライン警告を表示/非表示
    document.getElementById('is_famous_check')?.addEventListener('change', e => {
      const warning = document.getElementById('famousWarning');
  ...
  try {
    ...
    data.roles.forEach(r => {
      ...
      card.innerHTML = `
        ...
        <div>
          <div style="font-size:16px;color:var(--text-muted);font-weight:500;">${r.role}</div>
          <div style="font-size:14px;font-weight:600;color:var(--text-primary);">${r.persona_name}</div>
        </div>`;
...
function openAddModal() {
  ...
  // 著名人チェックとfamousWarningをリセット
  const isFamousCheck = document.getElementById('is_famous_check');
  const famousWarning = document.getElementById('famousWarning');
  if (isFamousCheck) isFamousCheck.checked = false;
  if (famousWarning) famousWarning.style.display = 'none';
  ...
  try {
    ...
    State.members.forEach(m => {
      if (m.avatar && m.avatar.startsWith('data:')) {
        ...
        const baseId = m.source_persona_id || m.id;
        if (!State.avatarImages[m.id] && DEFAULT_AVATARS[baseId]) {
          State.avatarImages[m.id] = DEFAULT_AVATARS[baseId];
        }
      }
    });
...
async function submitAddPersona() {
  ...
  // T-08: ペルソナ作成初回同意チェック
  if (!localStorage.getItem('persona_tos_agreed')) {
    State.addSubmitting = false;
    DOM.confirmAddPersona.disabled = false;
    DOM.confirmAddPersona.textContent = '追加する';
    openPersonaTosModal();
