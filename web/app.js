...
async function submitEditPersona() {
  ...
  try {
    // ★ バックエンドAPIでペルソナ情報を更新
    const data = await API.put(`/api/personas/${memberId}`, updateData);

    // フロントエンドのStateも更新
    const idx = State.members.findIndex(m => m.id === memberId);
    if (idx >= 0) {
      State.members[idx] = { ...State.members[idx], ...data.persona };
    }

    // 会議中の場合はセッション内メンバー情報も更新
    if (State.sessionId) {
      const sessionIdx = State.members.findIndex(m => m.id === memberId);
      if (sessionIdx >= 0) {
        State.members[sessionIdx] = { ...State.members[sessionIdx], ...data.persona };
      }
    }
